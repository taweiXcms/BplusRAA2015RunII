...
//  TFile*filePPReference=new TFile(Form("output%s.root",particle.Data()));
//  TGraphAsymmErrors*gaeBplusReference=(TGraphAsymmErrors*)filePPReference->Get(Form("gaeSigmaDecay%s",particle.Data()));
  
  TFile*filePPReference=new TFile(Form("ROOTfiles/CrossSectionPP.root"));  
  ...
  TH1F* hPtSigma = (TH1F*)filePPReference->Get("hPtSigma");
  TGraphAsymmErrors* gaeCrossSyst=(TGraphAsymmErrors*)filePPReference->Get(Form("gaeCrossSyst"));
...
//  double scalingfactor_ref=1e-6;
  double scalingfactor_ref=1e-6*208;
  double yvalue,xvalue,yerrorhigh,yerrorlow;
  double ycross,ycrosshigh,ycrosslow;
  ...
  for (int i=0;i<nbins;i++){
    ...
    gaeBplusReference->SetPoint(i,xvalue,yvalue*scalingfactor_ref);
    gaeBplusReference->SetPointEYhigh(i,yerrorhigh*scalingfactor_ref);
    gaeBplusReference->SetPointEYlow(i,yerrorlow*scalingfactor_ref);

    gaeCrossSyst->GetPoint(i,xvalue,ycross);
    ycrosshigh=gaeCrossSyst->GetEYhigh()[i];
    ycrosslow=gaeCrossSyst->GetEYlow()[i];
    gaeCrossSyst->SetPoint(i,xvalue,ycross*scalingfactor_ref);
    gaeCrossSyst->SetPointEYhigh(i,ycrosshigh*scalingfactor_ref);
    gaeCrossSyst->SetPointEYlow(i,ycrosslow*scalingfactor_ref);
    hPtSigma->SetBinContent(i+1,hPtSigma->GetBinContent(i+1)*scalingfactor_ref);
    hPtSigma->SetBinError(i+1,hPtSigma->GetBinError(i+1)*scalingfactor_ref);
  } 
  ...
  Double_t ySigmappStat[nbins];                 //y pp data stat err
  Double_t yPercSigmappStat[nbins];             //y pp data stat err
  Double_t ySigmappSysCenter[nbins];                 //y pp data sys err
  Double_t ySigmappSysHigh[nbins];                 //y pp data sys err
  Double_t ySigmappSysLow[nbins];                 //y pp data sys err
  Double_t yPercSigmappSysHigh[nbins];             //y pp data sys err
  Double_t yPercSigmappSysLow[nbins];             //y pp data sys err
  ...
  //uncomment if use pp data ref.
  for(Int_t i=0;i<nbins;i++) {
    yRefPP[i] = hPtSigma->GetBinContent(i+1);//change to real pp ref
	// add in pp stst unc.
    ySigmappStat[i] = hPtSigma->GetBinError(i+1);
    yPercSigmappStat[i] = ySigmappStat[i]/yRefPP[i];
    yPercSigmapPbStat[i] = sqrt(yPercSigmapPbStat[i]*yPercSigmapPbStat[i] + yPercSigmappStat[i]*yPercSigmappStat[i]); 
    ySigmapPbStat[i] = yPercSigmapPbStat[i]*ySigmapPb[i];
	
	// add in pp sys unc.
    gaeCrossSyst->GetPoint(i,xRefPP[i],ySigmappSysCenter[i]);
	ySigmappSysHigh[i] = gaeCrossSyst->GetEYhigh()[i];
	ySigmappSysLow[i] = gaeCrossSyst->GetEYlow()[i];
	yPercSigmappSysHigh[i] = ySigmappSysHigh[i]/ySigmappSysCenter[i];
	yPercSigmappSysLow[i] = ySigmappSysLow[i]/ySigmappSysCenter[i];
    yPercSigmapPbSystTotHigh[i] = sqrt(yPercSigmapPbSystTotHigh[i]*yPercSigmapPbSystTotHigh[i] + yPercSigmappSysHigh[i]*yPercSigmappSysHigh[i]);
    yPercSigmapPbSystTotLow[i] = sqrt(yPercSigmapPbSystTotLow[i]*yPercSigmapPbSystTotLow[i] + yPercSigmappSysLow[i]*yPercSigmappSysLow[i]);
    ySigmapPbSystTotHigh[i]=yPercSigmapPbSystTotHigh[i]*ySigmapPb[i];
    ySigmapPbSystTotLow[i]=yPercSigmapPbSystTotLow[i]*ySigmapPb[i];
  }
...
*/ 
  ...
  TFile *fout=new TFile(Form("RpA%s/fileRpA%s.root",particle.Data(),particle.Data()),"recreate");  
  fout->cd();
...
void NuclearModification_DrawOnSamePad_2015ppRef(){
...
*/
  cSigma->SaveAs(Form("RpABplus/canvasSigma.pdf"));  
  cRpA->SaveAs(Form("RpABplus/canvasRpA.pdf"));  
  cSigma->SaveAs(Form("RpABplus/canvasSigma.eps"));  
  cRpA->SaveAs(Form("RpABplus/canvasRpA.eps"));  
  cSigma->SaveAs(Form("RpABplus/canvasSigma.png"));  
  cRpA->SaveAs(Form("RpABplus/canvasRpA.png"));  
  cSigma->SaveAs(Form("RpABplus/canvasSigma.C"));  
  cRpA->SaveAs(Form("RpABplus/canvasRpA.C"));  
