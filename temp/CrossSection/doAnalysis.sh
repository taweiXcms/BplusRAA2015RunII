...
./fitBCent.exe 1 "$INPUTDATAPbPb"  "$INPUTMCPbPb"  "$TRGPbPb" "$CUTPbPb"   "$SELGENPbPb"   "$ISMCPbPb"   1   "$ISDOWEIGHTPbPb"   "$LABELPbPb"  "$OUTPUTFILEPbPb_CENT" "$NPFIT_PbPb" 0
rm fitBCent.exe
fi
...
if [ $DOANALYSISPP_PPEFF_FIT -eq 1 ]; then
...
if [ $DOANALYSISPP_PbPbEFF_FIT -eq 1 ]; then
g++ EfficiencyFit.C $(root-config --cflags --libs) -g -o EfficiencyFit.exe
...
g++ fitBCent.C $(root-config --cflags --libs) -g -o fitBCent.exe
./fitBCent.exe 1 "$INPUTDATAPbPb"  "$INPUTMCPbPb"  "$TRGPbPb" "$CUTPbPb"   "$SELGENPbPb"   "$ISMCPbPb"   1   "$ISDOWEIGHTPbPb"   "$LABELPbPb"  "$OUTPUTFILEPbPb_CENT_EFFCOR" "$NPFIT_PbPb" 1
rm fitBCent.exe
fi
