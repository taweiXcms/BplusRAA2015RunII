...
void MCefficiencyNew(int isPbPb, TString inputmc,
                     TString selmcgen, TString selmcgenacceptance, TString cut_recoonly, TString cut,
                     TString label, TString outputfile,
                     int useweight, Float_t centmin=0., Float_t centmax=100.)
{ 
  ...
  cout<<"  selmcgen = "<<endl;
  cout<<"    "<<selmcgen<<endl;
  cout<<"  selmcgenacceptance ="<<endl;
  cout<<"    "<<selmcgenacceptance<<endl;
  cout<<"  cut_recoonly ="<<endl;
  cout<<"    "<<cut_recoonly<<endl;
  cout<<"  cut ="<<endl;
  cout<<"    "<<cut<<endl;
  cout<<endl;

  cout<<"  weight option = "<<useweight<<endl;
  ...
  if(useweight==1)
    {
      ...
      weighpthat = "pthatweight";
      weightGpt = "(exp(0.599595+-0.492734*Gpt+0.0173053*Gpt*Gpt+-0.000261242*Gpt*Gpt*Gpt)+0.406017)*(pow(10,-0.107832+0.010248*Gpt+Gpt*Gpt*0.000079+Gpt*Gpt*Gpt*-0.000003+Gpt*Gpt*Gpt*Gpt*-0.000000+Gpt*Gpt*Gpt*Gpt*Gpt*0.000000))";
      weightBgenpt = "(exp(0.599595+-0.492734*Bgenpt+0.0173053*Bgenpt*Bgenpt+-0.000261242*Bgenpt*Bgenpt*Bgenpt)+0.406017)*(pow(10,-0.107832+0.010248*Bgenpt+Bgenpt*Bgenpt*0.000079+Bgenpt*Bgenpt*Bgenpt*-0.000003+Bgenpt*Bgenpt*Bgenpt*Bgenpt*-0.000000+Bgenpt*Bgenpt*Bgenpt*Bgenpt*Bgenpt*0.000000))";
  ...
  cout<<"    "<<weighpthat<<endl;
  cout<<"    "<<weightGpt<<endl;
  cout<<"    "<<weightBgenpt<<endl;
  cout<<"    "<<weightHiBin<<endl;
  cout<<endl;
  ...
  hemptyEff->Draw();
  hEff->Draw("same");
  canvasEff->SaveAs(Form("plotEffNew/canvasEff_study%s.pdf",Form(label.Data())));
  ...
  canvasPthat->SaveAs(Form("plotEffNew/canvasPthat_%s.pdf",Form(label.Data())));
  ...
  hemptySpectra->Draw();
  hPtGen->Draw("same");
  canvasSpectra->SaveAs(Form("plotEffNew/canvasSpectra_%s.pdf",Form(label.Data())));
...
*/  
  ...
  hemptySpectra->Draw(); 
  hPtMC->Draw("same");
  canvas1D->SaveAs(Form("plotEffNew/canvas1DhPtMC_%s.pdf",Form(label.Data())));
  canvas1D->Clear();
  ...
  canvas1D->SaveAs(Form("plotEffNew/canvas1DhPtMCrecoonly_%s.pdf",Form(label.Data())));
  canvas1D->Clear();
  ...
  hemptySpectra->Draw(); 
  hPtGen->Draw("same");
  canvas1D->SaveAs(Form("plotEffNew/canvas1DhPtGen_%s.pdf",Form(label.Data())));
  canvas1D->Clear();
  ...
  hemptySpectra->Draw(); 
  hPtGenAcc->Draw("same");
  canvas1D->SaveAs(Form("plotEffNew/canvas1DhPtGenAcc_%s.pdf",Form(label.Data())));
  canvas1D->Clear();
  ...
  hemptyEff->Draw(); 
  hEff->Draw("same");
  canvas1D->SaveAs(Form("plotEffNew/canvas1DhEff_%s.pdf",Form(label.Data())));
  canvas1D->Clear();
  ...
  hemptyEff->Draw(); 
  hEffReco->Draw("same");
  canvas1D->SaveAs(Form("plotEffNew/canvas1DhEffReco_%s.pdf",Form(label.Data())));
  canvas1D->Clear();
  ...
  hemptyEff->Draw(); 
  hEffAcc->Draw("same");
  canvas1D->SaveAs(Form("plotEffNew/canvas1DhEffAcc_%s.pdf",Form(label.Data())));
  canvas1D->Clear();
  ...
  canvas1D->SaveAs(Form("plotEffNew/canvas1DhEffSelection_%s.pdf",Form(label.Data())));
  canvas1D->Clear();
...
int main(int argc, char *argv[])
{
  ...
  if(argc == 12)
    MCefficiencyNew(atoi(argv[1]),argv[2],argv[3],argv[4],argv[5],argv[6],argv[7],argv[8],atoi(argv[9]),atof(argv[10]),atof(argv[11]));
  return 0;
}
