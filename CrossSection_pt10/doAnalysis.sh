...
./fitBY.exe 0 "$INPUTDATAPP"  "$INPUTMCPP"  "$TRGPP" "$CUTPP&&Bpt>10"  "$SELGENPP"   "$ISMCPP"   1   "$ISDOWEIGHTPP"   "$LABELPP"  "$OUTPUTFILEPP_Y" "$NPFIT_PP" 0
rm fitBY.exe
fi
...
g++ fitBY.C $(root-config --cflags --libs) -g -o fitBY.exe 
./fitBY.exe 1 "$INPUTDATAPbPb"  "$INPUTMCPbPb"  "$TRGPbPb" "$CUTPbPb&&Bpt>10"  "$SELGENPbPb"   "$ISMCPbPb"   1   "$ISDOWEIGHTPbPb"   "$LABELPbPb"  "$OUTPUTFILEPbPb_Y" "$NPFIT_PbPb" 0 "$CENTPbPbMIN" "$CENTPbPbMAX"
rm fitBY.exe
fi 
...
./MCefficiencyY.exe  0 "$INPUTMCPP"  "$SELGENPP&&Gpt>10&&Gpt<50" "$SELGENPPACCPP&&Gpt>10&&Gpt<50"  "$RECOONLYPP" "$CUTPP&&$TRGPPMC"  "$LABELPP" "$OUTPUTFILEMCSTUDYPP_Y" "$ISDOWEIGHTPP" "$CENTPbPbMIN" "$CENTPbPbMAX"
...
g++ MCefficiencyY.C $(root-config --cflags --libs) -g -o MCefficiencyY.exe
./MCefficiencyY.exe 1 "$INPUTMCPbPb"  "$SELGENPbPb&&Gpt>10&&Gpt<50" "$SELGENPbPbACCPbPb&&Gpt>10&&Gpt<50"  "$RECOONLYPbPb" "$CUTPbPb&&$TRGPbPbMC"  "$LABELPbPb" "$OUTPUTFILEMCSTUDYPbPb_Y" "$ISDOWEIGHTPbPb" "$CENTPbPbMIN" "$CENTPbPbMAX"
rm MCefficiencyY.exe
fi
...
./MCefficiencyCent.exe 1 "$INPUTMCPbPb"  "$SELGENPbPb&&Gpt>10&&Gpt<50" "$SELGENPbPbACCPbPb&&Gpt>10&&Gpt<50"  "$RECOONLYPbPb" "$CUTPbPb&&$TRGPbPbMC&&Bpt>10"  "$LABELPbPb" "$OUTPUTFILEMCSTUDYPbPb_CENT" "$ISDOWEIGHTPbPb" "$CENTPbPbMIN" "$CENTPbPbMAX"
